	...
	for {
		...
		switch leftIdx {
			case 0: // General & Output
				...
				cookieDisplay := T(*cfg, "cookies_mode_none")
				if cfg.CookiesMode == "file" {
					cookieDisplay = filepath.Base(cfg.CookiesFile)
					if cookieDisplay == "" || cookieDisplay == "." {
						cookieDisplay = "cookies.txt"
					}
				} else if cfg.CookiesMode == "browser" {
					cookieDisplay = strings.Title(cfg.CookiesBrowser)
				}
				...
				rightItems = []settingItem{
					...
					{Label: T(*cfg, "settings_cookies", cookieDisplay), CLI: "--cookies", Key: "cookies"},
...
func handleSettingEdit(s tcell.Screen, cfg *core.Config, key string) {
	switch key {
		...
		case "cookies":
			cOpts := []string{
				T(*cfg, "cookies_mode_file") + fmt.Sprintf(" [%s]", cfg.CookiesFile),
				T(*cfg, "cookies_mode_browser") + fmt.Sprintf(" [%s]", func() string {
					if cfg.CookiesBrowser == "" {
						return "chrome"
					}
					return cfg.CookiesBrowser
				}()),
				T(*cfg, "cookies_mode_none"),
			}
			cIdx := RunMenu(s, cfg, T(*cfg, "settings_title"), cOpts, "Configure Authentication Cookies:", T(*cfg, "footer_nav"))
			if cIdx == 0 {
				defPath := cfg.CookiesFile
				if defPath == "" {
					home, _ := os.UserHomeDir()
					defPath = filepath.Join(home, "Downloads", "cookies.txt")
				}
				if val, ok := TextInput(s, cfg, T(*cfg, "settings_title"), T(*cfg, "cookies_prompt_file"), defPath, T(*cfg, "footer_input")); ok {
					cfg.CookiesFile = strings.TrimSpace(val)
					cfg.CookiesMode = "file"
					_ = core.SaveConfig(*cfg)
				}
			} else if cIdx == 1 {
				var bLabels []string
				...
				bi := RunMenu(s, cfg, T(*cfg, "settings_title"), bLabels, T(*cfg, "cookies_prompt_browser"), T(*cfg, "footer_nav"))
				if bi >= 0 {
					cfg.CookiesBrowser = core.SupportedBrowsers[bi]
					cfg.CookiesMode = "browser"
					_ = core.SaveConfig(*cfg)
				}
			} else if cIdx == 2 {
